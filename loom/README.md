...
The other fifteen-and-one things in [`flow`](../) are emergent systems: simple
...
The seam has been crossed: [`glintveil`](../glintveil) (seed 7002),
[`fenchurn`](../fenchurn) (seed 7000) and [`saltcrawl`](../saltcrawl)
(seed 7001) were all proposed by loom, sat "on the loom" in the gallery, and
were then built by an agent that was handed the repo and told *do what you
want* — the registry's first `provenance: loom` entries with `status: built`.
The carrying-across was still human-shaped (someone had to hand over the
keys), but none of those three briefs was written by a person — and the agent
restocked the loom on its way out, so there is always cloth on it.
